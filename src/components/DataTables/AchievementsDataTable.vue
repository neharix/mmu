...
import { useAchievementsStore, useDataTableStore } from "@/stores/api.store.js";
...
const { deleteStatus, updateStatus, createStatus } = storeToRefs(achievementsStore);
...
const applySearch = () => {
  router.push({ name: 'achievements-list', query: { ...route.query, search: searchQuery.value } }).then(() => {
    emit('update')
  });
};
...
watch(deleteStatus, (newVal, oldVal) => {
  if (newVal) {
    if (newVal === 'success') {
      uxStore.addToast(t('deleteToast', { object: t('achievement') }), 'success');
...
watch(currentPage, (newVal) => {
  router.push({ name: 'achievements-list', query: { ...route.query, page: newVal } }).then(() => {
    emit('update')
  });
})
watch(sortColumn, (newVal) => {
  setTimeout(() => {
    router.push({ name: 'achievements-list', query: { ...route.query, order: sortOrder.value, column: newVal } }).then(() => {
      emit('update');
    })
  }, 50)
})
...
watch(sortOrder, (newVal) => {
  router.push({ name: 'achievements-list', query: { ...route.query, order: newVal, column: sortColumn.value } }).then(() => {
    emit('update');
  })
})
...
onMounted(async () => {
  if (updateStatus.value) {
    if (updateStatus.value === 'success') {
      uxStore.addToast(t("editToast", { object: t('achievement') }), 'success');
  ...
  if (createStatus.value) {
    if (createStatus.value === 'success') {
      uxStore.addToast(t("createToast", { object: t('achievement') }), 'success');
...
async function deleteItem() {
  await achievementsStore._delete(selectedItem.value);
  emit('update')
}
...
<template>
  ...
  <confirm-modal ref="deleteSelectedModal" @confirm="deleteItems('achievement', selectedItems)">
    <template #header>
      {{ $t('confirm') }}
    </template>
    <template #default>
          ...
          <div class="dropdown dropdown-end">
            ...
            <ul tabindex="0"
              ...
              <li class="px-4 py-2 select-none">{{ $t('objectsCount', {
                objectsCount: achievementsStore.objectsCount
              }) }}</li>
              ...
              <hr class="hr">
              <li>
                <button @click="dataTableStore.getExportedFile('achievement', selectedItems)"
        ...
        <thead class="bg-gray-200 dark:bg-[#113031]">
          <tr>
            ...
            <th
              ...
              {{ $t('maleCount').toUpperCase() }}
              <span :class="sortColumn === 'male_count' ? (sortOrder === 'asc' ? 'rotate-180' : '') : 'opacity-25'"
                class="ml-2 transition-transform duration-200 inline-block">
                ▲
              </span>
            </th>
            <th
              ...
              {{ $t('femaleCount').toUpperCase() }}
              <span :class="sortColumn === 'female_count' ? (sortOrder === 'asc' ? 'rotate-180' : '') : 'opacity-25'"
                class="ml-2 transition-transform duration-200 inline-block">
                ▲
              </span>
            </th>
              ...
              <div class="w-full flex items-center justify-center">
                <div class="inline-flex rounded-md shadow-xs" role="group">
                  <button type="button" :key="item.id"
                    @click="router.push({ name: 'edit-achievement', params: { id: item.id } })"
