...
import { useCountriesStore, useDataTableStore, useStudentsStore, useRegionsStore } from "@/stores/api.store.js";
...
watch(deleteStatus, (newVal, oldVal) => {
  if (newVal) {
    if (newVal === 'success') {
      uxStore.addToast(t('deleteToast', { object: t('student') }), 'success');
...
onMounted(async () => {
  ...
  if (updateStatus.value) {
    if (updateStatus.value === 'success') {
      uxStore.addToast(t("editToast", { object: t('student') }), 'success');
  ...
  if (createStatus.value) {
    if (createStatus.value === 'success') {
      uxStore.addToast(t("createToast", { object: t('student') }), 'success');
...
async function deleteItem() {
  await studentsStore._delete(selectedItem.value);
  emit('update')
}
...
<template>
  ...
  <confirm-modal ref="deleteSelectedModal" @confirm="deleteItems('student', selectedItems)">
    <template #header>
      {{ $t('confirm') }}
    </template>
    <template #default>
          ...
          <div class="dropdown dropdown-end">
            ...
            <ul tabindex="0"
              ...
              <li class="px-4 py-2 select-none">{{ $t('objectsCount', {
                objectsCount: studentsStore.objectsCount
              }) }}</li>
              ...
              <hr class="hr">
              <li>
                <button @click="dataTableStore.getExportedFile('student', selectedItems)"
              ...
              <div class="w-full flex items-center justify-center">
                <div class="inline-flex rounded-md shadow-xs" role="group">
                  <button type="button" :key="item.id"
                    @click="router.push({ name: 'edit-student', params: { id: item.id } })"
                  ...
                  <button type="button" :key="item.id"
                    @click="router.push({ name: 'about-student', params: { id: item.id } })"
